...
- **Stripe B2B Billing:** `artifacts/api-server/src/routes/stripe-enterprise.ts` — 5 endpoints: POST create-company (Stripe customer), POST subscribe (per-seat checkout at $12/seat/mo), POST update-seats (quantity adjustment), GET billing/:companyId (subscription status from DB with Stripe fallback), POST portal (Stripe billing portal). All auth-protected with `x-enterprise-key`. Subscription state synced to `companies` table (subscription_status, subscription_id, seat_count, billing_period_end).
- **Enterprise Webhooks:** `artifacts/api-server/src/lib/enterpriseWebhook.ts` — handles checkout.session.completed, customer.subscription.updated/deleted, invoice.payment_succeeded/failed. All events sync subscription state to DB and log to audit_logs. Endpoint: POST `/api/stripe-enterprise/webhook`.
