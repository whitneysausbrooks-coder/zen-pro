...
### 2026-04-30 — Build #9 enterprise-hardening pass (HOLD until Whitney's ship word)
Whitney chose **Option B**: do not ship Build #8, instead harden everything to enterprise-pilot grade and ship one Build #9. Zero defects, zero compromises. Build is built but **NOT dispatched** — waiting on Whitney's explicit ship word.

**T001 — API typecheck zero-error.** Pre-existing tsc errors (masked previously) all surgically fixed. Stripe SDK v20 API drift handled in `lib/enterpriseWebhook.ts`, `lib/billingReconciliation.ts`, `routes/stripe-enterprise.ts`: `Subscription.current_period_end` → `items.data[0].current_period_end`; `Invoice.subscription` → `Invoice.parent.subscription_details.subscription`; `Invoice.tax` → `sum(total_taxes[].amount)`; `invoices.retrieveUpcoming()` → `invoices.createPreview()`. Added `stripe@20.4.1` to api-server `package.json` dependencies. Express 5 `string | string[]` param coercion fixed via `String(req.params.id)`. TS7030 missing-returns fixed via `return void res.x()` pattern (84 occurrences across `sso-scim.ts`, `stripe-enterprise.ts`, `quest.ts`) — architect confirmed runtime-safe. Both api-server and mobile now `tsc --noEmit` clean.

**T002 — GDPR data-subject rights.** `GET /api/app-user/:id/export` and `POST /api/app-user/:id/delete` appended to `routes/app-user.ts`. Export returns full user data (profile + biometrics + AI personalization + ToS acceptances + auth events + sync errors + outcomes) as a download with `Content-Disposition: attachment`. Delete is anonymize-in-place (preserves PK so audit_logs / SET-NULL FKs survive). Both audit-log every call. **Architect blocker fixed:** both routes use `requireDeviceSignatureStrict` (defined inline) which HARD-401s on any non-`ok` signature — does NOT honor the global soft-mode rollout because regulated data must not leak. **Architect HIGH fixed:** placeholder email/name now `deleted_<random16hex>@neuroquest.local` (was `deleted_<userId>` — re-derivable from PK). `/register` now hard-blocks with 410 Gone if the PK is tombstoned (email starts with `deleted_`), preventing re-bind / data-resurrection.

**T003 — Mobile IAP auth bridge.** Server-side `routes/iap.ts` already routed `/validate`, `/restore`, `/entitlements` through `requireUserOrDevice` (Clerk OR HMAC device). Mobile `lib/iap.ts` already used `signedFetch`. Restored real `purchaseProduct` flow in `app/(tabs)/play.tsx` (was the "Free Daily Spins / coming soon" stub) — `handleBuySpinPack` now invokes the real IAP path with confirm Alert. Added post-purchase `fetchEntitlements` reconciliation in both `shop.tsx` and `play.tsx` so the server `spin_balance` becomes the source of truth for `SPINS_KEY` (defends against duplicate receipts and multi-device drift).

**T004 — Triple-weight pipeline verified.** Confirmed end-to-end: HealthKit/Health-Connect → `/api/app-user/biometrics` → `computeNeuroResilienceScore({hrv*0.5, sleep*0.35, strain*0.15})` → `app_user_biometrics.neuro_resilience_score` persistence → `/api/app-user/:id/baseline` read. The 7-day baseline gate in `routes/ai.ts` is **server-derived** from `app_user_biometrics` (COUNT, MIN(recorded_at), MIN_SAMPLES=5, MIN_BASELINE_DAYS=7) — never trusts client-supplied days. Smoke test: HRV=75, sleep=7h, strain=10 produced score 76 (classification "optimal"), matches expected weight math. README claim is consistent with code.

**T005 — Watch + Health smoke validation.** Mobile tsc clean. `lib/health.ts` permission paths all wrapped in try/catch (no Android-HC missing-permission crash). Server `wearableIntegration.ts` is source-agnostic (Fitbit/Garmin labels collapse to "wearable" enum, no source-specific switching that could crash on stub providers).

**T006 — README accuracy pass.** Section 7 compliance table downgraded HIPAA BAA from "Available on request for Enterprise tier" to "Roadmap, target Q4 2026 (BAA template under counsel review)" — Whitney does not have a signed BAA template yet, and the README must not promise vendor terms we cannot deliver. All other §7 claims now backed by code (GDPR routes exist, audit-logging exists, Apple JWS verification exists, sso-scim.ts typechecks clean as scaffold).

**T007 — Build, smoke, architect re-review.** Production CJS bundle (~1.5 MB) builds successfully. Smoke tests verified: 401 on unauth GDPR, 401 on unauth GDPR-strict, 200 on register/biometrics/export/delete round-trip, anonymized email visible after delete, 410 Gone on re-register of tombstoned PK, fresh PK still 200. Architect re-review (Apr 30 2026): initial run flagged 1 BLOCKER (soft-mode auth on GDPR routes) + 1 HIGH (re-derivable placeholders) — both fixed in the same session as described in T002 above. No remaining HIGH or BLOCKER findings.

**T008 — HOLD.** `eas build --auto-submit` NOT dispatched. Awaiting Whitney's explicit ship word.
