...
- **Database Tables (Billing):** `processed_stripe_events` (idempotency), `webhook_dead_letter` (DLQ with retry), `webhook_metrics` (observability), `billing_reconciliation_log` (drift tracking). Companies table extended: `dunning_attempts`, `dunning_last_at`, `suspended_at`, `grace_period_end`, `seat_cap`.
