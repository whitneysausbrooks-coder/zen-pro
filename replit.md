...
- **Compassion Wheel (Web):** Staggered stops 800/1300/1800ms, `winRef.current` avoids stale closures, interval-based symbol cycling, spring bounce on landing. Route: `/wellness`.
- **Lucky Wheel (Mobile):** SVG-based 12-segment spinning wheel replacing the old 3-reel slot machine. Segments: [50, 15, 20, 75, 15, 500, 25, 100, 10, 2x Boost, 30, 200]. Back-end determines winning index first, then front-end calculates exact rotation (segmentCenterAngle → targetAngle → no jitter) so pointer lands precisely on the correct segment. 2x Boost segment doubles the next donation. `onSpin(isWin, prizeValue, prizeLabel)` callback passes actual values to parent. Unmount-safe animation lifecycle with mountedRef guard.
