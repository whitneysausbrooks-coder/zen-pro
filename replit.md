...
- **ASC 606 Revenue Recognition:** `artifacts/api-server/src/lib/revenueRecognition.ts` — Full deferred revenue tracking for per-seat subscriptions. Daily pro-rata recognition based on billing period. Tracks: new subscriptions, invoice payments, seat changes (proration), cancellations, refunds. Revenue journal with double-entry style entries. Integrated into all webhook handlers (checkout completed, subscription updated/deleted, payment succeeded) and the update-seats endpoint. Endpoints: GET `/enterprise/revenue/summary` (real-time recognized vs deferred with per-company breakdown), GET `/enterprise/revenue/monthly` (12-month history with new bookings/cancellations/seat changes/refunds), GET `/enterprise/revenue/journal` (full journal with CSV export), POST `/enterprise/revenue/snapshot/:companyId` (point-in-time snapshot). All events audit-logged.
- **Admin Dashboard Revenue Panel:** Revenue Recognition (ASC 606) section showing: total contract value, recognized revenue, deferred revenue, % recognized with progress bar, per-company breakdown (seats, daily rate, recognized/deferred), monthly revenue breakdown (recognized, new bookings, churn, seat changes, refunds), revenue journal with color-coded entry types and CSV export link.
- **Database Tables (Billing):** `processed_stripe_events` (idempotency), `webhook_dead_letter` (DLQ with retry), `webhook_metrics` (observability), `billing_reconciliation_log` (drift tracking), `revenue_recognition` (point-in-time snapshots), `revenue_journal` (double-entry revenue events). Companies table extended: `dunning_attempts`, `dunning_last_at`, `suspended_at`, `grace_period_end`, `seat_cap`.
